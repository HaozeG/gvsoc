...
int main(){
    ...
    // uint64_t hbm_south_addr = ARCH_HBM_START_BASE + ARCH_HBM_NODE_ADDR_SPACE * (2 * ARCH_NUM_CLUSTER_Y + ARCH_NUM_CLUSTER_X);
    ...
    // When no bias used
    gemm_systolic_wise(n_token, inter_dim, dim, DATA_SIZE_BYTES, TILE_WIDTH_TOKENS, TILE_WIDTH_EXPERT_0, TILE_WIDTH_EXPERT_1, zomem(0));

    // When reading bias from HBM
    gemm_systolic_wise(n_token, inter_dim, dim, DATA_SIZE_BYTES, TILE_WIDTH_TOKENS, TILE_WIDTH_EXPERT_0, TILE_WIDTH_EXPERT_1, hbm_addr(test_bias_offset));
