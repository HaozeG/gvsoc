...
int main(){
    ...
    uint32_t in_token_addr = (uint32_t)(hbm_addr(0));
    // uint32_t n_token_addr = ((uint32_t *)(hbm_addr(4)))[0];
    // uint32_t dim_addr = ((uint32_t *)(hbm_addr(8)))[0];
    // uint32_t inter_dim_addr = ((uint32_t *)(hbm_addr(12)))[0];
    // uint32_t n_routed_experts_addr = ((uint32_t *)(hbm_addr(16)))[0];
    // uint32_t n_shared_experts_addr = ((uint32_t *)(hbm_addr(20)))[0];
    // uint32_t n_activated_experts_addr = ((uint32_t *)(hbm_addr(24)))[0];
    uint32_t gate_weights_addr = (uint32_t)(hbm_addr(4));
    uint32_t expert_w1_weights_addr = (uint32_t)(hbm_addr(8));
    uint32_t expert_w1_bias_addr = (uint32_t )(hbm_addr(12));
    uint32_t expert_w2_weights_addr = (uint32_t)(hbm_addr(16));
    uint32_t expert_w2_bias_addr = (uint32_t)(hbm_addr(20));
    uint32_t expert_w3_weights_addr = (uint32_t)(hbm_addr(24));
    uint32_t expert_w3_bias_addr = (uint32_t)(hbm_addr(28));
    uint32_t actual_out_addr = (uint32_t)hbm_addr(32);
    uint32_t golden_out_addr = (uint32_t)hbm_addr(36);
    ...
    // compute_moe(in_token_addr, n_token, dim, inter_dim, n_routed_experts, n_shared_experts, n_activated_experts, gate_weights_addr, expert_w1_weights_addr, expert_w1_bias_addr, expert_w2_weights_addr, expert_w2_bias_addr, expert_w3_weights_addr, expert_w3_bias_addr, actual_out_addr);
    flex_global_barrier_xy();
    if (flex_get_core_id() == 0 && flex_get_cluster_id() == 0) {
        flex_timer_end();
    }
    ...
    if (flex_is_first_core() && (flex_get_cluster_id()==0))
    {
        ...
        for (int i = 0; i < n_token; i++) {
            ...
            // for (int j = 0; j < dim; j++) {
            for (int j = 0; j < 8; j++) {
                printf("0x%04x ", ((uint16_t *)(hbm_addr(actual_out_addr)))[j + i * dim]);
            }
            printf("\n");
        }
