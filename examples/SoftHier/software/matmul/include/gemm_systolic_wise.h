...
{
    uint64_t HBM_offset_M;
    uint64_t HBM_offset_N;
    uint64_t HBM_offset_K;
    //General information
    uint64_t matrix_M;
    uint64_t matrix_N;
    uint64_t matrix_K;
    ...
    //Tile Information
    uint64_t tile_dimension_M;
    uint64_t tile_dimension_N;
    uint64_t tile_dimension_K;
    uint64_t elem_size;
    uint64_t tile_size_byte_X;
    uint64_t tile_size_byte_W;
    uint64_t tile_size_byte_Y;
    ...
    //Iteration
    uint64_t XW_tile_length;
    uint64_t Z_tile_on_row;
    uint64_t Z_tile_on_col;
    uint64_t Z_tile_all;
    uint64_t systolic_delay;

    uint64_t total_iter;

    //Current Actions
    uint64_t dma_runing;
    uint64_t redmule_runing;
...
GemmSystolicInfo gemm_systolic_wise_analysis(
    uint64_t M_size, uint64_t N_size, uint64_t K_size, uint64_t elem_size,
    uint64_t tile_dimension_M, uint64_t tile_dimension_N, uint64_t tile_dimension_K){
    ...
    uint64_t M_tile = (M_size + info.tile_dimension_M - 1)/info.tile_dimension_M;
    uint64_t N_tile = (N_size + info.tile_dimension_N - 1)/info.tile_dimension_N;
    uint64_t K_tile = (K_size + info.tile_dimension_K - 1)/info.tile_dimension_K;
...
void gemm_systolic_wise_compute_dma_access(GemmSystolicInfo * info, uint64_t iter){
    ...
    {
        uint64_t eff_iter = iter - info->systolic_delay;
        uint64_t xw_count = eff_iter;
        FlexPosition pos = get_pos(flex_get_cluster_id());
        ...
        uint64_t local_x = (xw_count%2 == 0)? info->X_offset_1 : info->X_offset_2;
        uint64_t local_w = (xw_count%2 == 0)? info->W_offset_1 : info->W_offset_2;
...
void gemm_systolic_wise_compute_redmule_action(GemmSystolicInfo * info, uint64_t iter){
    ...
    {
        uint64_t eff_iter = iter - info->systolic_delay - 1;
        uint64_t xw_count = eff_iter;
        info->use_redmule = 1;
...
void gemm_systolic_wise(
    uint64_t M_size, uint64_t N_size, uint64_t K_size, uint64_t elem_size,
    uint64_t tile_dimension_M, uint64_t tile_dimension_N, uint64_t tile_dimension_K, uint64_t *output_offset){

    flex_global_barrier_xy();
    uint64_t CID = flex_get_cluster_id();
